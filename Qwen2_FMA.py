...
with open("FMA_filtered_vocal_all.json") as f:
    meta = json.load(f)
results = []
for audio_path1, audio_path2, audio_path3, audio_path4 in tqdm(zip(meta[::4], meta[1::4], meta[2::4], meta[3::4]), 
                         total=len(meta)//4):
    ### first meta #######
    audio_full_path1 = os.path.join("FMA_audio_47s", audio_path1)
    ...
    ### second meta #######
    audio_full_path2 = os.path.join("FMA_audio_47s", audio_path2)
    # Define a simple conversation for music captioning with a local audio path
    conversation2 = [
        {'role': 'system', 'content': 'You are a helpful assistant for music captioning.'},
        {'role': 'user', 'content': [
     ...
     ### first meta #######
    audio_full_path3 = os.path.join("FMA_audio_47s", audio_path3)
    # Define a simple conversation for music captioning with a local audio path
    conversation3 = [
        {'role': 'system', 'content': 'You are a helpful assistant for music captioning.'},
        {'role': 'user', 'content': [
    ...
    ### second meta #######
    audio_full_path4 = os.path.join("FMA_audio_47s", audio_path4)
    # Define a simple conversation for music captioning with a local audio path
    conversation4 = [
        {'role': 'system', 'content': 'You are a helpful assistant for music captioning.'},
        {'role': 'user', 'content': [
    ...
    inputs = processor(text=text, audios=audios, return_tensors="pt", padding=True, sampling_rate=processor.feature_extractor.sampling_rate).to('cuda')
...
with open("Qwen_caption_FMA.json", "w") as f:
